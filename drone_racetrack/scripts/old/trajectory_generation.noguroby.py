...
class Trajectory():
    ...
    def QP_ObjMat(self, ord_poly, ord_deriv, mu):
        ...
        # (n_var × n_var) - dimensional real symmetric matrix Q
    ...
    def QP_Equ_Deriv0(self, ord_poly, QP):
        ...
        equ_mat_t1 = np.zeros( (self.traj['n'], n_coeffs) )
        equ_mat_t2 = np.zeros( (self.traj['n'], n_coeffs) )
        ...
        for i in range( self.traj['n'] ):
            ...
            equ_mat_t1[i, i * ord_poly : (i + 1) * ord_poly] = t_poly[i, :][np.newaxis, :]
            equ_mat_t2[i, i * ord_poly : (i + 1) * ord_poly] = t_poly[i + 1, :][np.newaxis, :]

            # Write space coordinate into vec
            equ_vec_t1[i] = self.poses[QP][i]
            equ_vec_t2[i] = self.poses[QP][i + 1]

        equ_mat_deriv0 = np.vstack( 
            (
                equ_mat_t1, 
                equ_mat_t2
            ) 
        ...
        return [equ_mat_deriv0, equ_vec_deriv0]
    ...
    def QP_Equ_Deriv_(self, ord_poly, ord_deriv, QP):
        ...
        equ_mat_deriv_ = np.zeros( ((self.traj['n'] - 1) * ord_dif, n_coeffs) )
        ...
        for j in range(0, ord_dif):
            ...
            for i in range( self.traj['n'] - 1 ):
                ...
                equ_mat_deriv_[row, cols1] = np.matmul(t_poly[i + 1], deriv_mat)
                equ_mat_deriv_[row, cols2] = -np.matmul(t_poly[i + 1], deriv_mat)

        # KITTING it ROUND WISE APPROACH OUTER KEYFRAMES
        equ_mat_deriv_outer_ = np.zeros( (ord_dif, n_coeffs) )
        ...
        for j in range(0, ord_dif):
            ...
            equ_mat_deriv_outer_[row, cols1] = np.matmul(t_poly[ 0 ], deriv_mat)
            equ_mat_deriv_outer_[row, cols2] = -np.matmul(t_poly[ - 1 ], deriv_mat)

        equ_mat_deriv_ = np.vstack( (equ_mat_deriv_, equ_mat_deriv_outer_) )
        ...
        return [equ_mat_deriv_, equ_vec_deriv_]
    ...
    def QP_Equ(self, ord_poly, ord_deriv, QP):
        ...
        equ_mat_deriv0, equ_vec_deriv0 = self.QP_Equ_Deriv0(ord_poly, QP)
        ...
        equ_mat_deriv_, equ_vec_deriv_ = self.QP_Equ_Deriv_(ord_poly, ord_deriv, QP)
    ...
    def QP_SolveCVXOPT(self, ord_poly, opt_mat, opt_vec=None, ine_mat=None, ine_vec=None, equ_mat=None, equ_vec=None):

        if opt_vec != None:
            opt_vec = cvxopt.matrix( opt_vec, tc='d' )
        else:
            opt_vec = cvxopt.matrix( np.zeros( (opt_mat.shape[0], ) ), tc='d' )

        opt_mat = cvxopt.matrix( opt_mat, tc='d' )
    ...
    # MORE PARAMTERES??? -> def MinSnapTraj(self, ord_poly):
    def MinSnapTraj(self):
        # Ressource ?????
        # Explanation ?????????
        ...
        # Here: ord_poly = 3 -> 1 + x + x²
        #ord_poly = 19 # ord_poly > ord_deriv_*
        ord_poly = 8
        # -------------------
        ...
        for QP in QPs:
            ...
            poly_coeffs = self.QP_SolveCVXOPT( 
                ord_poly,
                obj_mat, 
                equ_mat=equ_mat, 
                equ_vec=equ_vec
...
if __name__ == "__main__":
    ...
    x = np.array([      0,      1.5,        3,      2,      0,      -3,     -2,     0,      0       ], dtype=np.float64)
    y = np.array([      0,       1.5,        3,      4,      6,       3,     -5,     -4,      0,     ], dtype=np.float64)
    z = np.array([      0,      .1,         0.2,    0.2,    0.15,   0.13,   0.1,    0.07,   0,      ], dtype=np.float64)
    yaw = np.array([      0,      45,         90,     135,    180,    270,    290,    340,    0,      ], dtype=np.float64)
    t = np.array([      0,      1.0,    2,          3,      4,      5,      6,      7,      8       ] , dtype=np.float64)

    #x = np.hstack( (x[: -1], x))
    #y = np.hstack( (y[: -1], y))
    #z = np.hstack( (z[: -1], z))
    #yaw = np.hstack( (yaw [:-1], yaw))
    #t = np.hstack( (t[:-1], t + t[-1]))


    # *** \INPUTS  ***

    min_snap_traj = Trajectory( x=x, y=y, z=z, yaw=yaw, t=t, rounds=0)
    min_snap_traj.MinSnapTraj()    